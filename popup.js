...
function getCurrentDate(){
    ...
    const resultArray = dateCalculatorGR(day, month, year);                             // Calculates republican date
    const resultArray2 = formatRepDate(resultArray[0], resultArray[1], resultArray[2]); // Formats the calculated rep date
    ...
    document.getElementById("repdate").innerHTML=weekday + ", " + standardDay2 + " " + monthName + ", an " + standardYear2;

    const symbol = getSymb(day, month, year);
    document.getElementById("symbol").innerHTML=symbol;

    const monthPic = getPic(day, month, year);
    document.getElementById("monthimage").innerHTML= monthPic;
}
...
function dateCalculatorGR(day, month, year){
    ...
    const max = new Date(equinoxList[equinoxList.length - 1]); // Last day where calculations are possible with the dataset.

    if(input < start || input > max){
        return [null, null, null]; // This is invalid input and should not have been entered. This error will be addressed once this method proves successful for current date.
    }else{
        // The following loop's goal is to find the current year.
        let cYear = null;
...
function dateCalculatorRG(day, month, year){
    if(year < 1 || year > equinoxList.length){
        return [null, null, null]; // This is invalid input and should not have been entered. This error will be addressed once this method proves successful for current date.
    }else{
...
function formatRepDate(day, month, year){
    // Takes in input according to republican calendar.
...
function formatGregDate(day, month, year){
    // Takes in input according to gregorian calendar.
    ...
    return [weekdayNames[dayIndex],day,monthNames[month-1],year];
}

function getSymb(day, month, year){
    // Takes in input according to republican calendar.
    const ruralCalendar = [
    ...
    const resultArray = dateCalculatorGR(day, month, year);           // Runs the date calculator for input.
...
function convertToRepublican(){
    ...
    const resultArray2 = formatRepDate(resultArray[0], resultArray[1], resultArray[2]);
    const weekday = resultArray2[0];                             // Unpacks the output values.
    const monthName = resultArray2[2];                           // "

    const symbol = getSymb(day, month, year);
    
    document.getElementById("formatted").innerHTML=weekday + ", " + standardDay + " " + monthName + ", an " + standardYear + " | " + symbol;
};
...
function convertToGregorian(){
    ...
    const resultArray2 = formatGregDate(resultArray[0], resultArray[1], resultArray[2]);
    const weekday = resultArray2[0];
    const monthName = resultArray2[2];
